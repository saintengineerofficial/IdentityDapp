...
const SocialMediaInputs: React.FC<SocialMediaInputsProps> = ({ formData, handleChange, errors }) => (
  <>
    <FormItem className="items-center justify-center w-full">
      <FormLabel className="text-sm">{errors?.x && <p className="text-red-500 text-xs mt-1">{errors?.x}</p>}X (Formerly Twitter)</FormLabel>
      <FormControl>
        <Input placeholder="https://x.com/johndoe" onChange={e => handleChange("x", e.target.value)} value={formData.x} />
      </FormControl>
    </FormItem>
